(** * equivcpp.v : C++-faithful rendering of the infinity-aware helpers.
    ...
    [itv.v]'s [mul_zinf] instead spells out all nine constructor pairs, which
...
From LalaInterval Require Import Concrete Zinf itv zdiv.
Open Scope Z_scope.
...
Definition ztdiv_pos3 (s : store3) : store3 :=
  ...
  let z :=
    ...
    else Itv (max_zinf (lb z) (cdiv_zinf (addk_zinf (ub y) (-1)) (addk_zinf (lb x) (-1)))) (ub z) in
  (* Z: tymax(x.ub, z) >= y.lb *)
  let z :=
    if geq0_zinf (ub x) then
      Itv (max_zinf (lb z) (cdiv_zinf (addk_zinf (lb y) 1) (addk_zinf (ub x) 1))) (ub z)
    else Itv (lb z) (min_zinf (ub z) (fdiv_zinf (lb y) (ub x))) in
  if negb (nonempty3b z) then St3 x y z else
...
Lemma z1_pos_ob : forall s vy vz,
  1 <= vz -> leq_zinf (lb (sx3 s)) (Fin (Z.quot vy vz)) -> leq_zinf (Fin vy) (ub (sy3 s)) ->
  ispos_zinf (lb (sx3 s)) = true ->
  leq_zinf (Fin vz) (fdiv_zinf (ub (sy3 s)) (lb (sx3 s))).
Proof.
  intros s vy vz Hvz Hxl Hyu Hzp.
  destruct (lb (sx3 s)) as [a| |] eqn:Ex; cbn in Hzp; try discriminate; try (cbn in Hxl; contradiction).
  (* lb x = Fin a, 0 < a *)
...
Lemma z1_neg_ob : forall s vy vz,
  1 <= vz -> leq_zinf (lb (sx3 s)) (Fin (Z.quot vy vz)) -> leq_zinf (Fin vy) (ub (sy3 s)) ->
  ispos_zinf (lb (sx3 s)) = false ->
  leq_zinf (cdiv_zinf (addk_zinf (ub (sy3 s)) (-1)) (addk_zinf (lb (sx3 s)) (-1))) (Fin vz).
Proof.
  intros s vy vz Hvz Hxl Hyu Hzp.
  destruct (lb (sx3 s)) as [a| |] eqn:Ex; cbn in Hzp; try discriminate.
  - (* Fin a, a <= 0 *)
...
Lemma z2_pos_ob : forall s vy vz,
  1 <= vz -> leq_zinf (Fin (Z.quot vy vz)) (ub (sx3 s)) -> leq_zinf (lb (sy3 s)) (Fin vy) ->
  geq0_zinf (ub (sx3 s)) = true ->
  leq_zinf (cdiv_zinf (addk_zinf (lb (sy3 s)) 1) (addk_zinf (ub (sx3 s)) 1)) (Fin vz).
Proof.
  intros s vy vz Hvz Hxu Hyl Hzg.
  destruct (ub (sx3 s)) as [b| |] eqn:Ex; cbn in Hzg; try discriminate; try (cbn in Hxu; contradiction).
  - (* Fin b, 0 <= b *)
...
Lemma z2_neg_ob : forall s vy vz,
  1 <= vz -> leq_zinf (Fin (Z.quot vy vz)) (ub (sx3 s)) -> leq_zinf (lb (sy3 s)) (Fin vy) ->
  geq0_zinf (ub (sx3 s)) = false ->
  leq_zinf (Fin vz) (fdiv_zinf (lb (sy3 s)) (ub (sx3 s))).
Proof.
  intros s vy vz Hvz Hxu Hyl Hzg.
  destruct (ub (sx3 s)) as [b| |] eqn:Ex; cbn in Hzg; try discriminate; try (cbn in Hxu; contradiction).
  (* Fin b, b < 0 *)
...
Lemma ylo_pos : forall s vy vz iz,
  ...
  leq_zinf (lb (sx3 s)) (Fin (Z.quot vy vz)) -> ispos_zinf (lb (sx3 s)) = true ->
...
Lemma ylo_neg : forall s vy vz iz,
  1 <= vz -> leq_zinf (Fin 1) (lb iz) -> leq_zinf (lb iz) (Fin vz) -> leq_zinf (Fin vz) (ub iz) ->
  leq_zinf (lb (sx3 s)) (Fin (Z.quot vy vz)) -> ispos_zinf (lb (sx3 s)) = false ->
  leq_zinf (min_zinf (addk_zinf (mul_zinf (addk_zinf (lb (sx3 s)) (-1)) (lb iz)) 1)
            (addk_zinf (mul_zinf (addk_zinf (lb (sx3 s)) (-1)) (ub iz)) 1)) (Fin vy).
Proof.
  intros s vy vz iz Hvz Hz1 Hzl Hzu Hxl Hzp.
  destruct (lb iz) as [zl| |]; cbn in Hz1, Hzl; try contradiction.
  destruct (lb (sx3 s)) as [a| |] eqn:Ex; cbn in Hzp; try discriminate.
...
Lemma yhi_pos : forall s vy vz iz,
  1 <= vz -> leq_zinf (Fin 1) (lb iz) -> leq_zinf (lb iz) (Fin vz) -> leq_zinf (Fin vz) (ub iz) ->
  leq_zinf (Fin (Z.quot vy vz)) (ub (sx3 s)) -> geq0_zinf (ub (sx3 s)) = true ->
...
Lemma yhi_neg : forall s vy vz iz,
  1 <= vz -> leq_zinf (Fin 1) (lb iz) -> leq_zinf (lb iz) (Fin vz) -> leq_zinf (Fin vz) (ub iz) ->
  leq_zinf (Fin (Z.quot vy vz)) (ub (sx3 s)) -> geq0_zinf (ub (sx3 s)) = false ->
  leq_zinf (Fin vy) (max_zinf (mul_zinf (ub (sx3 s)) (lb iz)) (mul_zinf (ub (sx3 s)) (ub iz))).
Proof.
  intros s vy vz iz Hvz Hz1 Hzl Hzu Hxu Hzg.
  destruct (lb iz) as [zl| |]; cbn in Hz1, Hzl; try contradiction.
  destruct (ub (sx3 s)) as [b| |] eqn:Ex; cbn in Hzg; try discriminate; try (cbn in Hxu; contradiction).
...
Lemma step_hilo : forall (C:bool) lo hi Bh Bl vz,
  ...
  mem3 (if C then Itv lo (min_zinf hi Bh) else Itv (max_zinf lo Bl) hi) vz.
Proof.
...
Lemma step_lohi : forall (C:bool) lo hi Bl Bh vz,
  ...
  mem3 (if C then Itv (max_zinf lo Bl) hi else Itv lo (min_zinf hi Bh)) vz.
Proof.
...
Lemma step_hilo_pos : forall (C:bool) lo hi Bh Bl,
  leq_zinf (Fin 1) lo ->
  leq_zinf (Fin 1) (lb (if C then Itv lo (min_zinf hi Bh) else Itv (max_zinf lo Bl) hi)).
...
Lemma step_lohi_pos : forall (C:bool) lo hi Bl Bh,
  leq_zinf (Fin 1) lo ->
  leq_zinf (Fin 1) (lb (if C then Itv (max_zinf lo Bl) hi else Itv lo (min_zinf hi Bh))).
...
Lemma step_hilo_lo : forall (C:bool) lo hi Bh Bl,
  leq_zinf lo (lb (if C then Itv lo (min_zinf hi Bh) else Itv (max_zinf lo Bl) hi)).
...
Lemma step_hilo_hi : forall (C:bool) lo hi Bh Bl,
  leq_zinf (ub (if C then Itv lo (min_zinf hi Bh) else Itv (max_zinf lo Bl) hi)) hi.
Proof. intros; destruct C; cbn [ub]; [apply min_zinf_leq_zinf_l | apply leq_zinf_refl]. Qed.
Lemma step_lohi_lo : forall (C:bool) lo hi Bl Bh,
  leq_zinf lo (lb (if C then Itv (max_zinf lo Bl) hi else Itv lo (min_zinf hi Bh))).
...
Lemma step_lohi_hi : forall (C:bool) lo hi Bl Bh,
  leq_zinf (ub (if C then Itv (max_zinf lo Bl) hi else Itv lo (min_zinf hi Bh))) hi.
Proof. intros; destruct C; cbn [ub]; [apply leq_zinf_refl | apply min_zinf_leq_zinf_l]. Qed.
...
Lemma ispos_zinf3_band : forall s,
  ...
  exists vz, 1 <= vz /\ mem3 (sz3 s) vz
    /\ leq_zinf (tyminZ (lb (sx3 s)) vz) (ub (sy3 s))
    /\ leq_zinf (lb (sy3 s)) (tymaxZ (ub (sx3 s)) vz).
Proof.
  intros s Hx Hy Hne.
  ...
  - (* Z2 band: lo y <= tymaxZ (hi x) zv *)
    destruct (ub (sx3 s)) as [b| |] eqn:Exu;
...
Proof.
  ...
  pose proof (band_ne (lb (sx3 s)) (ub (sx3 s)) vz Hvz1 Hxle) as Hbn.
  pose proof (nonempty_bounds _ Hx) as [Hxlp Hxun].
  pose proof (nonempty_bounds _ Hy) as [Hylp Hyun].
  set (L := max_zinf (tyminZ (lb (sx3 s)) vz) (lb (sy3 s))).
  set (U := min_zinf (tymaxZ (ub (sx3 s)) vz) (ub (sy3 s))).
  ...
  - split; [ | split ].
    + split.
      ...
      * apply quot_le; [exact Hvz1 | eapply leq_zinf_trans; [exact HvyU | apply min_zinf_leq_zinf_l]].
    + split.
...
Lemma wit_in_t : forall s t vz vy,
  ...
  leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin vy) -> leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) vz) ->
...
Proof.
  ...
  assert (Hband : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
    leq_zinf (tyminZ (lb (sx3 s)) vz) (ub (sy3 s)) /\ leq_zinf (lb (sy3 s)) (tymaxZ (ub (sx3 s)) vz)).
  { intros vz Hvlo Hvhi. split.
    - destruct (lb (sx3 s)) as [a| |] eqn:Exl; [ | congruence | cbn [tyminZ]; exact I ].
      destruct (Z.lt_ge_cases 0 a) as [Ha|Ha].
      + assert (Hzp : ispos_zinf (Fin a) = true) by (cbn; apply Z.ltb_lt; lia).
      ...
      + assert (Hzp : ispos_zinf (Fin a) = false) by (cbn; apply Z.ltb_ge; lia).
        ...
        rewrite Hz1; cbn [lb]. apply leq_zinf_max_zinf_r.
    - destruct (ub (sx3 s)) as [b| |] eqn:Exu; [ | cbn [tymaxZ]; apply leq_zinf_Pinf | congruence ].
      destruct (Z.lt_ge_cases 0 b) as [Hb|Hb].
      + assert (Hzg : geq0_zinf (Fin b) = true) by (cbn; apply Z.leb_le; lia).
  ...
  assert (Hwit : forall vz vy, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
    mem3 (sy3 s) vy -> leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin vy) -> leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) vz) ->
    mem3 (sx3 t) (Z.quot vy vz) /\ mem3 (sy3 t) vy /\ mem3 (sz3 t) vz).
  ...
  assert (Hpickvy : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
    exists vy, mem3 (sy3 s) vy /\ leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin vy) /\ leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) vz)).
  { intros vz Hvlo Hvhi.
    destruct (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
    pose proof (band_ne (lb (sx3 s)) (ub (sx3 s)) vz (Hv1 vz Hvlo) Hxle) as Hbn.
    set (L := max_zinf (tyminZ (lb (sx3 s)) vz) (lb (sy3 s))).
    set (U := min_zinf (tymaxZ (ub (sx3 s)) vz) (ub (sy3 s))).
    assert (HLU : leq_zinf L U).
    { unfold L, U. apply leq_zinf_min_zinf_glb.
  ...
  assert (Hattx : forall vz vy, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
    leq_zinf (lb (sy3 s)) (Fin vy) -> leq_zinf (Fin vy) (ub (sy3 s)) ->
    leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin vy) -> leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) vz) ->
    leq_zinf (lb (sx3 t)) (Fin (Z.quot vy vz)) /\ leq_zinf (Fin (Z.quot vy vz)) (ub (sx3 t))).
  { intros vz vy A1 A2 A3 A4 A5 A6.
  ...
  - (* ===== X-COMPONENT (goal 1) ===== *)
    assert (Hlyf : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
      leq_zinf (lb yF) (tymaxZ (ub (sx3 s)) vz)).
    { intros vz Hvlo Hvhi.
      ...
      - destruct (Z.lt_ge_cases 0 a) as [Ha|Ha].
        + assert (Hzp : ispos_zinf (Fin a) = true) by (cbn; apply Z.ltb_lt; lia).
          ...
          eapply leq_zinf_trans; [ apply min_zinf_leq_zinf_l | cbn; nia ].
        + assert (Hzp : ispos_zinf (Fin a) = false) by (cbn; apply Z.ltb_ge; lia).
  ...
  apply ile3_intro; cbn [lb ub].
    { (* --- x-lower --- *)
      assert (Hcx : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
        forall ylv, lb yF = Fin ylv -> leq_zinf (lb (sx3 s)) (Fin (Z.quot ylv vz)) ->
        leq_zinf (lb (sx3 t)) (Fin (Z.quot ylv vz))).
      { intros vz Hvlo Hvhi ylv Eyl Hxc.
        assert (Hbl : leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin ylv)).
        { destruct (lb (sx3 s)) as [a| |] eqn:Exl; cbn [tyminZ].
          - cbn in Hxc. cbn. apply (tymin_lb a ylv vz); [pose proof (Hv1 vz Hvlo); lia | exact Hxc].
          - cbn in Hxc. contradiction.
          - exact I. }
        assert (Hbu : leq_zinf (Fin ylv) (tymaxZ (ub (sx3 s)) vz)) by (rewrite <- Eyl; apply (Hlyf vz Hvlo Hvhi)).
      ...
      destruct (leqb_zinf (lb (sx3 s)) (min_zinf (min_zinf (idivt3 (lb yF) (lb z2)) (idivt3 (lb yF) (ub z2)))
      ...
      - apply leq_zinf_prop_bool_equiv in Hclip.
        ...
        + assert (Hcll : leq_zinf (lb (sx3 s)) (idivt3 (Fin ylv) (lb z2))).
          ...
          apply leq_zinf_min_zinf_glb.
          * eapply leq_zinf_trans; [apply min_zinf_leq_zinf_l | rewrite Hzlo2; apply idivt3_mono_pos; [exact Hyfne | lia] ].
        ...
        + assert (En : idivt3 Ninf (Fin zlo2) = Ninf) by (cbn [idivt3]; rewrite (proj2 (Z.ltb_lt 0 zlo2) ltac:(lia)); reflexivity).
          ...
          assert (Hbl : leq_zinf (tyminZ (lb (sx3 s)) zlo2) (Fin vy)) by (rewrite Hxn; cbn; exact I).
          assert (Hbu : leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) zlo2)) by (eapply leq_zinf_trans; [ | exact Hby2]; cbn; unfold vy; lia).
      ...
      - eapply leq_zinf_trans; [ | apply leq_zinf_max_zinf_l ].
        ...
        assert (Hclipx : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
          leq_zinf (idivt3 (lb yF) (Fin vz)) (Fin (xl - 1)) -> leq_zinf (lb (sx3 t)) (Fin xl)).
        { intros vz Hvlo Hvhi Hcorner.
          pose proof (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
          destruct (attain_vx (Fin xl) (ub (sx3 s)) vz xl ltac:(pose proof (Hv1 vz Hvlo); lia) (leq_zinf_refl _) Hxle) as (Hbl & Hbu & Hqeq).
    ...
    { (* --- x-upper --- *)
      assert (Hhyf : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
        leq_zinf (tyminZ (lb (sx3 s)) vz) (ub yF)).
      { intros vz Hvlo Hvhi.
        pose proof (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
        pose proof (band_ne (lb (sx3 s)) (ub (sx3 s)) vz (Hv1 vz Hvlo) Hxle) as Hbn.
        assert (Hvzge : zlo2 <= vz) by (pose proof Hvlo as HH; rewrite Hzlo2 in HH; cbn in HH; lia).
      ...
      assert (Hcxu : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
        forall yhv, ub yF = Fin yhv -> leq_zinf (Fin (Z.quot yhv vz)) (ub (sx3 s)) ->
        leq_zinf (Fin (Z.quot yhv vz)) (ub (sx3 t))).
      { intros vz Hvlo Hvhi yhv Eyh Hxc.
        assert (Hbu : leq_zinf (Fin yhv) (tymaxZ (ub (sx3 s)) vz)).
        { destruct (ub (sx3 s)) as [b| |] eqn:Exu; cbn [tymaxZ].
          - cbn in Hxc. cbn. apply (tymax_ub b yhv vz); [pose proof (Hv1 vz Hvlo); lia | exact Hxc].
          - exact I.
          - cbn in Hxc. contradiction. }
        assert (Hbl : leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin yhv)) by (rewrite <- Eyh; apply (Hhyf vz Hvlo Hvhi)).
      ...
      destruct (leqb_zinf (max_zinf (max_zinf (idivt3 (lb yF) (lb z2)) (idivt3 (lb yF) (ub z2)))
         (max_zinf (idivt3 (ub yF) (lb z2)) (idivt3 (ub yF) (ub z2)))) (ub (sx3 s))) eqn:Hclip.
      - apply leq_zinf_prop_bool_equiv in Hclip.
        eapply leq_zinf_trans; [ apply min_zinf_leq_zinf_r | ].
        destruct (ub yF) as [yhv| |] eqn:Eyh.
        + assert (Hchl : leq_zinf (idivt3 (Fin yhv) (lb z2)) (ub (sx3 s))).
          { eapply leq_zinf_trans; [ | exact Hclip ]. eapply leq_zinf_trans; [ | apply leq_zinf_max_zinf_r]. apply leq_zinf_max_zinf_l. }
          assert (Hchh : leq_zinf (idivt3 (Fin yhv) (ub z2)) (ub (sx3 s))).
          { eapply leq_zinf_trans; [ | exact Hclip ]. eapply leq_zinf_trans; [ | apply leq_zinf_max_zinf_r]. apply leq_zinf_max_zinf_r. }
          assert (HA : leq_zinf (max_zinf (idivt3 (Fin yhv) (lb z2)) (idivt3 (Fin yhv) (ub z2))) (ub (sx3 t))).
          { apply max_zinf_lub.
        ...
        + assert (Hzp : forall x, max_zinf x Pinf = Pinf) by (intros [?| |]; reflexivity).
          ...
          assert (Hbl : leq_zinf (tyminZ (lb (sx3 s)) zlo2) (Fin vy)) by (eapply leq_zinf_trans; [exact Hby1 | ]; cbn; unfold vy; lia).
          assert (Hbu : leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) zlo2)) by (rewrite Hxp; cbn; exact I).
          destruct (Hwit zlo2 vy Hzlo2lo Hzlo2hi Hmemy Hbl Hbu) as (Hmxt & _ & _).
      ...
      - eapply leq_zinf_trans; [ apply min_zinf_leq_zinf_l | ].
        ...
        assert (Hclipxu : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
          leq_zinf (Fin (xu + 1)) (idivt3 (ub yF) (Fin vz)) -> leq_zinf (Fin xu) (ub (sx3 t))).
        { intros vz Hvlo Hvhi Hcorner.
          pose proof (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
          destruct (attain_vx_hi (lb (sx3 s)) (Fin xu) vz xu ltac:(pose proof (Hv1 vz Hvlo); lia) Hxle (leq_zinf_refl _)) as (Hbl & Hbu & Hqeq).
          ...
          + cbn [ispos_zinf] in Eylo. destruct (Z.lt_ge_cases 0 a) as [Ha|Ha].
            ...
            * rewrite (proj2 (Z.ltb_ge 0 a) Ha) in Eylo. rewrite Hzlo2 in Eylo.
              ...
              -- set (vz := Z.max zlo2 (Z.max 1 (2 - M))).
                 ...
                 assert (Htymax : leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) vz)).
                 { pose proof (band_ne (Fin a) (ub (sx3 s)) vz (Hv1 vz HB1) Hxle) as Hbn.
              ...
              -- congruence.
          + congruence.
          + destruct (tymaxZ (ub (sx3 s)) zlo2) as [tm| |] eqn:Etm.
            * set (vy := Z.min (M-1) tm).
              assert (Hmemy : mem3 (sy3 s) vy).
              ...
              assert (Htymax : leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) zlo2)) by (rewrite Etm; cbn; unfold vy; lia).
            ...
            * set (vy := M-1).
              ...
              { split; [rewrite Hlo0; exact I | eapply leq_zinf_trans'; [ | exact Hfyhi]; cbn; unfold vy; lia]. }
              assert (Htymin : leq_zinf (tyminZ Ninf zlo2) (Fin vy)) by (cbn; exact I).
              assert (Htymax : leq_zinf (Fin vy) (tymaxZ (ub (sx3 s)) zlo2)) by (rewrite Etm; apply leq_zinf_Pinf).
              destruct (Hwit zlo2 vy Hzlo2lo Hzlo2hi Hmemy Htymin Htymax) as (_ & Hmyt & _).
              pose proof (mem3_lo (sy3 t) vy Hmyt) as HH. rewrite EtL in HH. cbn in HH. unfold vy in HH. lia.
            * exfalso. apply (tymaxZ_not_Ninf (ub (sx3 s)) zlo2 Hxun). exact Etm. }
      assert (Hex : exists vzs, leq_zinf (lb z2) (Fin vzs) /\ leq_zinf (Fin vzs) (ub z2) /\
        leq_zinf (tyminZ (lb (sx3 s)) vzs) (Fin vlo) /\ leq_zinf (Fin vlo) (tymaxZ (ub (sx3 s)) vzs)).
      { destruct (lb (sx3 s)) as [a| |] eqn:Exl.
        - destruct (Z.lt_ge_cases 0 a) as [Ha|Ha].
          + exists zlo2. split; [exact Hzlo2lo | split; [exact Hzlo2hi | ]].
            pose proof (Hband zlo2 Hzlo2lo Hzlo2hi) as [Hb1 Hb2].
            pose proof (band_ne (Fin a) (ub (sx3 s)) zlo2 Hzlo21 Hxle) as Hbn.
          ...
          + destruct (ub z2) as [zhi2| |] eqn:Eh.
            * assert (Hzhi2lo : leq_zinf (lb z2) (Fin zhi2)) by exact Hz2ne.
              ...
              pose proof (band_ne (Fin a) (ub (sx3 s)) zhi2 (Hv1 zhi2 Hzhi2lo) Hxle) as Hbn.
              assert (Hzp : ispos_zinf (Fin a) = false) by (cbn; apply Z.ltb_ge; lia).
          ...
          + cbn [geq0_zinf] in Eyhi. destruct (Z.le_gt_cases 0 xu) as [Hu|Hu].
            * rewrite (proj2 (Z.leb_le 0 xu) Hu) in Eyhi. rewrite Hzlo2 in Eyhi.
              ...
              -- set (vz := Z.max zlo2 (Z.max 1 (M + 2))).
                 ...
                 assert (Htymin : leq_zinf (tyminZ (lb (sx3 s)) vz) (Fin vy)).
                 { pose proof (band_ne (lb (sx3 s)) (Fin xu) vz (Hv1 vz HB1) Hxle) as Hbn.
            ...
            * rewrite (proj2 (Z.leb_gt 0 xu) Hu) in Eyhi. rewrite Hzlo2 in Eyhi.
              ...
              -- congruence.
          + destruct (tyminZ (lb (sx3 s)) zlo2) as [tmn| |] eqn:Etm.
            * set (vy := Z.max (M+1) tmn).
              ...
              assert (Htymin : leq_zinf (tyminZ (lb (sx3 s)) zlo2) (Fin vy)) by (rewrite Etm; cbn; unfold vy; lia).
              destruct (Hwit zlo2 vy Hzlo2lo Hzlo2hi Hmemy Htymin Htymax) as (_ & Hmyt & _).
              pose proof (mem3_hi (sy3 t) vy Hmyt) as HH. rewrite EtH in HH. cbn in HH. unfold vy in HH. lia.
            * exfalso. apply (tyminZ_not_Pinf (lb (sx3 s)) zlo2 Hxlp). exact Etm.
            * set (vy := M+1).
              assert (Hmemy : mem3 (sy3 s) vy).
              { split; [eapply leq_zinf_trans'; [exact Hfylo | ]; cbn; unfold vy; lia | rewrite Hhi0; exact I]. }
              assert (Htymax : leq_zinf (Fin vy) (tymaxZ Pinf zlo2)) by (cbn [tymaxZ]; apply leq_zinf_Pinf).
              assert (Htymin : leq_zinf (tyminZ (lb (sx3 s)) zlo2) (Fin vy)) by (rewrite Etm; cbn; exact I).
              destruct (Hwit zlo2 vy Hzlo2lo Hzlo2hi Hmemy Htymin Htymax) as (_ & Hmyt & _).
              pose proof (mem3_hi (sy3 t) vy Hmyt) as HH. rewrite EtH in HH. cbn in HH. unfold vy in HH. lia.
          + congruence. }
      2:{ pose proof (nonempty_bounds _ EY) as [_ HH]; congruence. }
      assert (Hexu : exists vzs, leq_zinf (lb z2) (Fin vzs) /\ leq_zinf (Fin vzs) (ub z2) /\
        leq_zinf (tyminZ (lb (sx3 s)) vzs) (Fin vhi) /\ leq_zinf (Fin vhi) (tymaxZ (ub (sx3 s)) vzs)).
      { destruct (ub (sx3 s)) as [xu| |] eqn:Exu.
        - destruct (Z.le_gt_cases 0 xu) as [Hu|Hu].
          + destruct (ub z2) as [zhi2| |] eqn:Eh.
            * assert (Hzhi2lo : leq_zinf (lb z2) (Fin zhi2)) by exact Hz2ne.
              exists zhi2. split; [exact Hzhi2lo | split; [apply leq_zinf_refl | ]].
              pose proof (Hband zhi2 Hzhi2lo (leq_zinf_refl _)) as [Hb1 Hb2].
              pose proof (band_ne (lb (sx3 s)) (Fin xu) zhi2 (Hv1 zhi2 Hzhi2lo) Hxle) as Hbn.
          ...
          + exists zlo2. split; [exact Hzlo2lo | split; [exact Hzlo2hi | ]].
            pose proof (Hband zlo2 Hzlo2lo Hzlo2hi) as [Hb1 Hb2].
            pose proof (band_ne (lb (sx3 s)) (Fin xu) zlo2 Hzlo21 Hxle) as Hbn.
...
(* [itv.v] already provides the extended tests [ispos_zinf] ("> 0"), [isneg_zinf] ("< 0")
...
(* the compact version is exactly [itv.v]'s [mul_zinf] *)
...
(** The C++ compact form (precondition: b <> 0):
    ...
    The "a > 0"/"a < 0" tests are [itv.v]'s extended [ispos_zinf]/[isneg_zinf].

    [itv.v]'s [cdiv_zinf] spells out the constructor pairs; here we prove the
...
(* the compact version is exactly [itv.v]'s [cdiv_zinf]; the finite/infinite
...
(** The C++ compact form (precondition: b <> 0):
    ...
    The "a < 0"/"a > 0" tests are [itv.v]'s extended [isneg_zinf]/[ispos_zinf].

    [itv.v]'s [fdiv_zinf] spells out the constructor pairs; here we prove the
    compact rendering is the SAME function. *)
...
(* the compact version is exactly [itv.v]'s [fdiv_zinf] *)

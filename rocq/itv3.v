...
(* ================================================================== *)
(** ** Part 2 : compressed finite floored-division  (was fdiv2.v)      *)
(* ================================================================== *)
...
Proof.
  ...
  destruct (Z.ltb_spec 0 (lo iz)) as [Hpos|Hnpos]; cbv beta iota.
  - (* positive side: every w in the refined band satisfies
       lo ix * w <= hi iy  and  lo iy + 1 <= (hi ix + 1) * w *)
    match goal with |- context[if ?g then _ else _] => destruct g eqn:Hk end;
      cbn [lo hi].
    + (* kill row: empty result contradicts nonemptiness *)
      intro Hne; exfalso; lia.
    + apply Bool.orb_false_iff in Hk; destruct Hk as [Hk1 Hk2];
        apply Bool.andb_false_iff in Hk1; apply Bool.andb_false_iff in Hk2.
      assert (Hk1' : lo ix = 0 -> 0 <= hi iy).
      { intro; destruct Hk1 as [F|F];
          [apply Z.eqb_neq in F; lia | apply Z.ltb_ge in F; lia]. }
      assert (Hk2' : hi ix + 1 = 0 -> lo iy < 0).
      { intro; destruct Hk2 as [F|F];
          [apply Z.eqb_neq in F; lia | apply Z.leb_gt in F; lia]. }
      clear Hk1 Hk2.
      destruct (Z.ltb_spec (lo ix) 0) as [Ha0|Ha0];
        destruct (Z.ltb_spec 0 (lo ix)) as [Ha1|Ha1]; try (exfalso; lia);
        destruct (Z.ltb_spec 0 (hi ix + 1)) as [Hb0|Hb0];
        destruct (Z.ltb_spec (hi ix + 1) 0) as [Hb1|Hb1]; try (exfalso; lia);
        cbv beta iota; intro Hne.
      * (* lo ix < 0, hi ix + 1 > 0 *)
        pose proof (cdiv_mul_le_neg (hi iy) (lo ix) Ha0) as PA.
        pose proof (cdiv_mul_ge (lo iy + 1) (hi ix + 1) Hb0) as PB.
        match type of Hne with ?L <= ?H =>
          assert (HW : forall w, L <= w -> w <= H ->
            exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
        { intros w Hwl Hwh.
          assert (Hw0 : 0 < w) by lia.
          assert (HA : lo ix * w <= hi iy) by nia.
          assert (HB : lo iy <= (hi ix + 1) * w - 1) by nia.
          destruct (witness_pos (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
            as (x & y & Hx & Hy & Hz0 & Hxy).
          exists x, y; unfold mem, sol; tauto. }
        split; apply HW; lia.
      * (* lo ix < 0, hi ix + 1 < 0 *)
        pose proof (cdiv_mul_le_neg (hi iy) (lo ix) Ha0) as PA.
        pose proof (div_mul_ge_neg (lo iy + 1) (hi ix + 1) Hb1) as PB.
        match type of Hne with ?L <= ?H =>
          assert (HW : forall w, L <= w -> w <= H ->
            exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
        { intros w Hwl Hwh.
      ...
      * (* lo ix < 0, hi ix + 1 = 0 *)
        pose proof (cdiv_mul_le_neg (hi iy) (lo ix) Ha0) as PA.
        assert (PB : lo iy < 0) by (apply Hk2'; lia).
        assert (Hbz : hi ix + 1 = 0) by lia.
        match type of Hne with ?L <= ?H =>
          assert (HW : forall w, L <= w -> w <= H ->
            exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
        { intros w Hwl Hwh.
          assert (Hw0 : 0 < w) by lia.
          assert (HA : lo ix * w <= hi iy) by nia.
          assert (HB : lo iy <= (hi ix + 1) * w - 1) by (rewrite Hbz; lia).
          destruct (witness_pos (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
            as (x & y & Hx & Hy & Hz0 & Hxy).
          exists x, y; unfold mem, sol; tauto. }
        split; apply HW; lia.
      * (* lo ix > 0, hi ix + 1 > 0 *)
        pose proof (Z.mul_div_le (hi iy) (lo ix) Ha1) as PA.
        pose proof (cdiv_mul_ge (lo iy + 1) (hi ix + 1) Hb0) as PB.
        match type of Hne with ?L <= ?H =>
          assert (HW : forall w, L <= w -> w <= H ->
            exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
        ...
        split; apply HW; lia.
      * (* lo ix = 0, hi ix + 1 > 0 *)
        assert (PA : 0 <= hi iy) by (apply Hk1'; lia).
        assert (Haz : lo ix = 0) by lia.
        pose proof (cdiv_mul_ge (lo iy + 1) (hi ix + 1) Hb0) as PB.
        match type of Hne with ?L <= ?H =>
          assert (HW : forall w, L <= w -> w <= H ->
            exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
        { intros w Hwl Hwh.
          assert (Hw0 : 0 < w) by lia.
          assert (HA : lo ix * w <= hi iy) by (rewrite Haz; lia).
          assert (HB : lo iy <= (hi ix + 1) * w - 1) by nia.
          destruct (witness_pos (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
            as (x & y & Hx & Hy & Hz0 & Hxy).
          exists x, y; unfold mem, sol; tauto. }
        split; apply HW; lia.
  - destruct (Z.ltb_spec (hi iz) 0) as [Hneg|Hnneg]; cbv beta iota.
    + (* negative side: every w in the refined band satisfies
         lo iy <= lo ix * w  and  (hi ix + 1) * w < hi iy *)
      match goal with |- context[if ?g then _ else _] => destruct g eqn:Hk end;
        cbn [lo hi].
      * (* kill row *)
        intro Hne; exfalso; lia.
      * apply Bool.orb_false_iff in Hk; destruct Hk as [Hk1 Hk2];
          apply Bool.andb_false_iff in Hk1; apply Bool.andb_false_iff in Hk2.
        assert (Hk1' : lo ix = 0 -> lo iy <= 0).
        { intro; destruct Hk1 as [F|F];
            [apply Z.eqb_neq in F; lia | apply Z.ltb_ge in F; lia]. }
        assert (Hk2' : hi ix + 1 = 0 -> 0 < hi iy).
        { intro; destruct Hk2 as [F|F];
            [apply Z.eqb_neq in F; lia | apply Z.leb_gt in F; lia]. }
        clear Hk1 Hk2.
        destruct (Z.ltb_spec (lo ix) 0) as [Ha0|Ha0];
          ...
          cbv beta iota; intro Hne.
        -- (* lo ix < 0, hi ix + 1 > 0 *)
           pose proof (div_mul_ge_neg (lo iy) (lo ix) Ha0) as PA.
           pose proof (Z.mul_div_le (hi iy - 1) (hi ix + 1) Hb0) as PB.
           match type of Hne with ?L <= ?H =>
             assert (HW : forall w, L <= w -> w <= H ->
               exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
           { intros w Hwl Hwh.
             assert (Hw0 : w < 0) by lia.
             assert (HA : lo iy <= lo ix * w) by nia.
             assert (HB : (hi ix + 1) * w < hi iy) by nia.
             destruct (realize_neg (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
               as (x & y & Hx & Hy & Hz0 & Hxy).
             exists x, y; unfold mem, sol; tauto. }
           split; apply HW; lia.
        -- (* lo ix < 0, hi ix + 1 < 0 *)
           pose proof (div_mul_ge_neg (lo iy) (lo ix) Ha0) as PA.
           pose proof (cdiv_mul_le_neg (hi iy - 1) (hi ix + 1) Hb1) as PB.
           match type of Hne with ?L <= ?H =>
             assert (HW : forall w, L <= w -> w <= H ->
               exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
           { intros w Hwl Hwh.
        ...
        -- (* lo ix < 0, hi ix + 1 = 0 *)
           pose proof (div_mul_ge_neg (lo iy) (lo ix) Ha0) as PA.
           assert (PB : 0 < hi iy) by (apply Hk2'; lia).
           assert (Hbz : hi ix + 1 = 0) by lia.
           match type of Hne with ?L <= ?H =>
             assert (HW : forall w, L <= w -> w <= H ->
               exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
           { intros w Hwl Hwh.
             assert (Hw0 : w < 0) by lia.
             assert (HA : lo iy <= lo ix * w) by nia.
             assert (HB : (hi ix + 1) * w < hi iy) by (rewrite Hbz; lia).
             destruct (realize_neg (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
               as (x & y & Hx & Hy & Hz0 & Hxy).
             exists x, y; unfold mem, sol; tauto. }
           split; apply HW; lia.
        -- (* lo ix > 0, hi ix + 1 > 0 *)
           pose proof (cdiv_mul_ge (lo iy) (lo ix) Ha1) as PA.
           pose proof (Z.mul_div_le (hi iy - 1) (hi ix + 1) Hb0) as PB.
           match type of Hne with ?L <= ?H =>
             assert (HW : forall w, L <= w -> w <= H ->
               exists x0 y0, mem ix x0 /\ mem iy y0 /\ sol x0 y0 w) end.
           ...
           split; apply HW; lia.
        -- (* lo ix = 0, hi ix + 1 > 0 *)
           assert (PA : lo iy <= 0) by (apply Hk1'; lia).
           assert (Haz : lo ix = 0) by lia.
           pose proof (Z.mul_div_le (hi iy - 1) (hi ix + 1) Hb0) as PB.
           match type of Hne with ?L <= ?H =>
             assert (HW : forall w, L <= w -> w <= H ->
           ...
           { intros w Hwl Hwh.
             assert (Hw0 : w < 0) by lia.
             assert (HA : lo iy <= lo ix * w) by (rewrite Haz; lia).
             assert (HB : (hi ix + 1) * w < hi iy) by nia.
             destruct (realize_neg (lo ix) (hi ix) (lo iy) (hi iy) w Hw0 Hxne Hyne HA HB)
               as (x & y & Hx & Hy & Hz0 & Hxy).
             exists x, y; unfold mem, sol; tauto. }
           split; apply HW; lia.
    + (* straddling iz: excluded by the sign hypothesis *)
      exfalso; destruct Hsign as [Hs|Hs]; lia.
Qed.
...
Proof.
  intro t; unfold restrict_z_pos3; apply sle3_intro; cbn [sx3 sy3 sz3];
    [ apply ile3_refl | apply ile3_refl | apply inter3_ile3_l ].
Qed.

Lemma restrict_z_neg3_ile : forall t, sle3 (restrict_z_neg3 t) t.
Proof.
  intro t; unfold restrict_z_neg3; apply sle3_intro; cbn [sx3 sy3 sz3];
    [ apply ile3_refl | apply ile3_refl | apply inter3_ile3_l ].
Qed.

Lemma fdivxz3_ile : forall w, sle3 (fdivxz3 w) w.
Proof.
  intro w; unfold fdivxz3; cbv zeta; apply sle3_intro; cbn [sx3 sy3 sz3];
    [ eapply ile3_trans; [apply inter3_ile3_l | apply inter3_ile3_l]
    | apply ile3_refl
    | apply inter3_ile3_l ].
Qed.
...
Proof.
  intro w; unfold refine_y3; cbv zeta; apply sle3_intro; cbn [sx3 sy3 sz3];
    [ apply ile3_refl | apply inter3_ile3_l | apply ile3_refl ].
Qed.
...
Proof.
  ...
  (* each sign-definite branch, if non-empty, forces the exact solution *)
  assert (Bpos : ne_store3 (fdivxz3 (restrict_z_pos3 t)) = true -> sol vx vy vz).
  { intro HB.
    ...
    split; [lia | exact Hq]. }
  assert (Bneg : ne_store3 (fdivxz3 (restrict_z_neg3 t)) = true -> sol vx vy vz).
  { intro HB.
    pose proof (fdivxz3_ne_z _ HB) as Hzz.

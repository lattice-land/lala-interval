...
Proof. intro i; split; apply leq_zitv_reflexivity. Qed.
...
Proof. intros i j k [H1 H2] [H3 H4]; split; eapply leq_zitv_transitivity; eassumption. Qed.

(* antisymmetry of [leq_zitv] holds exactly up to [ieq] (that IS [ieq]). *)
Lemma leq_zitv3_antisym_mod : forall i j, leq_zitv i j -> leq_zitv j i -> ieq i j.
Proof. intros i j H1 H2; split; assumption. Qed.
...
(* infimum by the order-reversing bijection [neg_zinf]. *)
...
Proof.
  intro S. destruct (zlub_exists (fun a => S (neg_zinf a))) as [M [Hub Hle]].
  exists (neg_zinf M). split.
  - intros b Hb.
    assert (HS' : S (neg_zinf (neg_zinf b))) by (rewrite neg_zinf_invol; exact Hb).
    pose proof (Hub (neg_zinf b) HS') as H.
    apply (proj2 (neg_zinf_le M (neg_zinf b))) in H.
    rewrite neg_zinf_invol in H. exact H.
  - intros l Hl.
    assert (Hubl : zub (fun a => S (neg_zinf a)) (neg_zinf l)).
    { intros a Ha. pose proof (Hl (neg_zinf a) Ha) as H.
      apply (proj2 (neg_zinf_le (neg_zinf a) l)) in H. rewrite neg_zinf_invol in H. exact H. }
    pose proof (Hle (neg_zinf l) Hubl) as H.
    apply (proj2 (neg_zinf_le (neg_zinf l) M)) in H.
    rewrite neg_zinf_invol in H. exact H.
Qed.
...
Lemma leq_zitv3_iqcup_l : forall i j, leq_zitv i (iqcup i j).
Proof.
  intros i j. unfold iqcup. destruct (is_not_bot_zitv i) eqn:Ei.
  - destruct (is_not_bot_zitv j) eqn:Ej.
    + apply leq_zitv_intro; unfold join_nobot_zitv; cbn [lb ub]; [ apply leq_zinf_min_zinf_l | apply leq_zinf_max_zinf_l ].
    + apply leq_zitv_reflexivity.
  - apply bot_is_leq_all_zitv; exact Ei.
Qed.

Lemma leq_zitv3_iqcup_r : forall i j, leq_zitv j (iqcup i j).
Proof.
  intros i j. unfold iqcup. destruct (is_not_bot_zitv i) eqn:Ei.
  - destruct (is_not_bot_zitv j) eqn:Ej.
    + apply leq_zitv_intro; unfold join_nobot_zitv; cbn [lb ub]; [ apply leq_zinf_min_zinf_r | apply leq_zinf_max_zinf_r ].
...
Proof.
  ...
  - destruct (is_not_bot_zitv j) eqn:Ej.
    + apply ijoin_leq_zitv3; assumption.
    + exact Hik.
  - exact Hjk.
Qed.

(* the defining absorption laws: order = meet-fixed = join-fixed (up to ~) *)
Lemma leq_zitv3_meet : forall i j, leq_zitv i j <-> ieq (meet_zitv i j) i.
Proof.
  intros i j; split.
  - intro H. split; [ apply inter_leq_zitv_l | apply inter_glb; [ apply leq_zitv_reflexivity | exact H ] ].
  - intros [_ H]. eapply leq_zitv_transitivity; [ exact H | apply inter_leq_zitv_r ].
Qed.

Lemma leq_zitv3_join : forall i j, leq_zitv i j <-> ieq (iqcup i j) j.
Proof.
  intros i j; split.
  - intro H. split; [ apply iqcup_lub; [ exact H | apply leq_zitv_reflexivity ] | apply leq_zitv3_iqcup_r ].
  - intros [H _]. eapply leq_zitv_transitivity; [ apply leq_zitv3_iqcup_l | exact H ].
Qed.
...
Lemma leq_zitv3_itop : forall i, leq_zitv i itop.
Proof. intro i. apply leq_zitv_intro; [ apply leq_zinf_ninf_l | apply leq_zinf_pinf_r ]. Qed.
Lemma leq_zitv3_ibot : forall i, leq_zitv ibot i.
Proof. intro i. apply bot_is_leq_all_zitv. reflexivity. Qed.

Lemma leq_zitv3_bot_absorb : forall l e, leq_zitv l e -> is_not_bot_zitv e = false -> is_not_bot_zitv l = false.
Proof.
...
Definition leq_zitv3_lb (S : zitv -> Prop) (m : zitv) : Prop := forall i, S i -> leq_zitv m i.
Definition leq_zitv3_ub (S : zitv -> Prop) (m : zitv) : Prop := forall i, S i -> leq_zitv i m.
Definition is_iglb (S : zitv -> Prop) (m : zitv) : Prop :=
  leq_zitv3_lb S m /\ (forall l, leq_zitv3_lb S l -> leq_zitv l m).
Definition is_ilub (S : zitv -> Prop) (m : zitv) : Prop :=
  leq_zitv3_ub S m /\ (forall u, leq_zitv3_ub S u -> leq_zitv m u).
...
Proof.
  ...
  - exists ibot. split.
    + intros i Hi. apply leq_zitv3_ibot.
    + intros l Hl. apply bot_is_leq_all_zitv. exact (leq_zitv3_bot_absorb l e (Hl e He) Hee).
...
Proof.
  ...
  - exists ibot. split.
    ...
    + intros u Hu. apply leq_zitv3_ibot.
Qed.

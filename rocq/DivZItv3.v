(** * div4: the four division propagators with INFINITE bounds
    ...
    solvers are needed ([fdiv_pos_z_zitv3] and [tdiv_pos_z_zitv3], mirroring the C++
    ...
    The zinf interval infrastructure ([zinf], [zitv], [zitv3], the
...
Definition slice_feasible (P : Z -> Z -> Z -> Prop) (s : zitv3) : Prop :=
  exists vx vy vz, in_zitv3 s vx vy vz /\ P vx vy vz /\ 1 <= vz.
Definition slice_contains (P : Z -> Z -> Z -> Prop) (s t : zitv3) : Prop :=
  forall vx vy vz, in_zitv3 s vx vy vz -> P vx vy vz -> 1 <= vz ->
  in_zitv3 t vx vy vz.

(* ================================================================== *)
(** Support lemmas for the truncated positive-slice soundness proof
    ([tdiv_pos_z_zitv3_soundness]).  Reusable arithmetic bridges (floor/ceil vs Z.quot),
...
Proof.
  ...
  destruct (lb (x s)) as [a| |] eqn:Ex; cbn in Hxl; try contradiction.
...
Proof.
  ...
  destruct (lb iz) as [zl| |]; cbn in Hz1, Hzl; try contradiction.
  destruct (ub (x s)) as [b| |] eqn:Ex; cbn in Hxu; try contradiction.
...
Proof.
  ...
  - destruct (iszero_zinf (lb (x s))) eqn:Ez; cbn [negb].
    ...
    + (* lo-narrow : lo x < 0 or Ninf *)
      ...
      destruct (lb (x s)) as [a| |] eqn:Ex; cbn in Ezp, Ez, Hxl; try discriminate.
      * (* Fin a, a < 0 *)
...
Proof.
  ...
  - destruct (eqm1_zinf (ub (x s))) eqn:Ee; cbn [negb].
    ...
    + (* hi-narrow : hi x < 0, != -1, or Ninf *)
      apply contains_narrow_hi; [exact Hz0l | exact Hz0u | ].
      destruct (ub (x s)) as [b| |] eqn:Ex; cbn in Ezg, Ee, Hxu; try discriminate; try contradiction.
      * (* Fin b, b < 0, b <> -1 *)
...
Proof.
  ...
  - (* vz <= -1 : negative slice via neg_yz_zitv3 *)
    ...
    pose proof (fdiv_pos_z_zitv3_soundness (neg_yz_zitv3 s) vx (- vy) (- vz) Hmir Hsmir ltac:(lia)) as Hp.
  ...
  - (* vz >= 1 : positive slice directly *)
    apply in_join4_l.
    apply fdiv_pos_z_zitv3_soundness; [exact Hin | split; [exact Hnz | exact Hq] | lia].
Qed.
...
Proof.
  ...
  - (* vz <= -1 : ceil(y/z) = -floor(y/(-z)) via neg_xz_zitv3 *)
    ...
    pose proof (fdiv_pos_z_zitv3_soundness (neg_xz_zitv3 s) (- vx) vy (- vz) Hmir Hsmir ltac:(lia)) as Hp.
  ...
  - (* vz >= 1 : ceil(y/z) = -floor((-y)/z) via neg_xy_zitv3 *)
    ...
    pose proof (fdiv_pos_z_zitv3_soundness (neg_xy_zitv3 s) (- vx) (- vy) vz Hmir Hsmir ltac:(lia)) as Hp.
...
Lemma sol_mir : forall vx vy vz, fdiv_rel vx vy vz -> fdiv_rel vx (- vy) (- vz).
Proof.
...
Proof.
  ...
  assert (Hsol : fdiv_rel (vy / vz) vy vz) by (split; [lia | reflexivity]).
...
Proof.
  ...
  - destruct zl as [l| |]; cbn in Hz1, Hzl; try contradiction.
    eapply leq_zinf_trans; [apply leq_zinf_min_zinf_l | ]. rewrite mul_zinf_ninf_fp by lia. exact I.
Qed.
...
Proof.
  ...
  { eapply nonempty_is_not_bot_zitv3. apply fdiv_pos_z_zitv3_soundness; [exact Hfin | exact Hfts | exact Hfz1]. }
  ...
  { intros vz Hvlo Hvhi.
    ...
    apply (flo_ycorner_lo (lb (x s)) (lb z2) (ub z2) vz Hz2l1 Hvlo Hvhi). }
  assert (Hhyf : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
    leq_zinf (fyminZ (lb (x s)) vz) (ub yF)).
  { intros vz Hvlo Hvhi.
    pose proof (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
    pose proof (fband_ne (lb (x s)) (ub (x s)) vz (Hv1 vz Hvlo) Hxle) as Hbn.
    rewrite HyF; cbn [ub]. apply leq_zinf_min_zinf_glb; [exact Hb1 | ].
    eapply leq_zinf_trans; [ exact Hbn | ].
    apply (flo_ycorner_hi (ub (x s)) (lb z2) (ub z2) vz Hz2l1 Hvlo Hvhi). }
  ...
  { assert (Hcx : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
      forall ylv, lb yF = Fin ylv -> leq_zinf (lb (x s)) (Fin (ylv / vz)) ->
      leq_zinf (lb (x t)) (Fin (ylv / vz))).
    { intros vz Hvlo Hvhi ylv Eyl Hxc.
      assert (Hbl : leq_zinf (fyminZ (lb (x s)) vz) (Fin ylv)).
      { destruct (lb (x s)) as [a| |] eqn:Exl; cbn [fyminZ].
    ...
    - eapply leq_zinf_trans; [ | apply leq_zinf_max_zinf_l ].
      ...
      assert (Hclipx : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
        leq_zinf (fdiv_zinf (lb yF) (Fin vz)) (Fin (xl - 1)) -> leq_zinf (lb (x t)) (Fin xl)).
  ...
  { assert (Hcxu : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
      forall yhv, ub yF = Fin yhv -> leq_zinf (Fin (yhv / vz)) (ub (x s)) ->
      leq_zinf (Fin (yhv / vz)) (ub (x t))).
    { intros vz Hvlo Hvhi yhv Eyh Hxc.
      assert (Hbu : leq_zinf (Fin yhv) (fymaxZ (ub (x s)) vz)).
      { destruct (ub (x s)) as [b| |] eqn:Exu; cbn [fymaxZ].
    ...
    - apply leq_zinf_prop_bool_equiv in Hclip.
      ...
      + assert (Hchl : leq_zinf (fdiv_zinf (Fin yhv) (lb z2)) (ub (x s))).
        ...
        assert (Hchh : leq_zinf (fdiv_zinf (Fin yhv) (ub z2)) (ub (x s))).
        { eapply leq_zinf_trans; [ | exact Hclip ]. eapply leq_zinf_trans; [ | apply leq_zinf_max_zinf_r]. apply leq_zinf_max_zinf_r. }
        assert (HA : leq_zinf (max_zinf (fdiv_zinf (Fin yhv) (lb z2)) (fdiv_zinf (Fin yhv) (ub z2))) (ub (x t))).
        { apply max_zinf_lub.
    ...
    - eapply leq_zinf_trans; [ apply leq_zinf_min_zinf_l | ].
      ...
      assert (Hclipxu : forall vz, leq_zinf (lb z2) (Fin vz) -> leq_zinf (Fin vz) (ub z2) ->
        leq_zinf (Fin (xu + 1)) (fdiv_zinf (ub yF) (Fin vz)) -> leq_zinf (Fin xu) (ub (x t))).
      { intros vz Hvlo Hvhi Hcorner.
        pose proof (Hband vz Hvlo Hvhi) as [Hb1 Hb2].
  ...
  { destruct (lb yF) as [vlo| |] eqn:Eylo.
    - assert (Hex : exists vzs, leq_zinf (lb z2) (Fin vzs) /\ leq_zinf (Fin vzs) (ub z2) /\
        leq_zinf (fyminZ (lb (x s)) vzs) (Fin vlo) /\ leq_zinf (Fin vlo) (fymaxZ (ub (x s)) vzs)).
      { destruct (lb (x s)) as [a| |] eqn:Exl.
        - destruct (Z.le_gt_cases 0 a) as [Ha|Ha].
          + exists zlo2. split; [exact Hzlo2lo | split; [exact Hzlo2hi | ]].
            ...
            pose proof (fband_ne (Fin a) (ub (x s)) zlo2 Hzlo21 Hxle) as Hbn.
          ...
          + destruct (ub z2) as [zhi2| |] eqn:Eh.
            * exists zhi2. split; [exact Hz2ne | split; [apply leq_zinf_refl | ]].
              ...
              pose proof (fband_ne (Fin a) (ub (x s)) zhi2 (Hv1 zhi2 Hz2ne) Hxle) as Hbn.
        ...
        * destruct (ub z2) as [zh| |] eqn:Eh.
          ...
          -- destruct (ub (y s)) as [hh| |] eqn:Ehy.
             ++ set (vzs := Z.max zlo2 (Z.max 1 (Z.max (1-M) (1-hh)))).
                ...
                { pose proof (fband_ne (Fin a) (ub (x s)) vzs Hz1v Hxle) as Hbn.
  ...
  { destruct (ub yF) as [vhi| |] eqn:Eyhi.
    - assert (Hexu : exists vzs, leq_zinf (lb z2) (Fin vzs) /\ leq_zinf (Fin vzs) (ub z2) /\
        leq_zinf (fyminZ (lb (x s)) vzs) (Fin vhi) /\ leq_zinf (Fin vhi) (fymaxZ (ub (x s)) vzs)).
      { destruct (ub (x s)) as [xu| |] eqn:Exu.
        - destruct (Z.le_gt_cases 0 xu) as [Hu|Hu].
          + destruct (ub z2) as [zhi2| |] eqn:Eh.
            * exists zhi2. split; [exact Hz2ne | split; [apply leq_zinf_refl | ]].
              pose proof (Hband zhi2 Hz2ne (leq_zinf_refl _)) as [Hb1 Hb2].
              pose proof (fband_ne (lb (x s)) (Fin xu) zhi2 (Hv1 zhi2 Hz2ne) Hxle) as Hbn.
      ...
      + destruct (Z.le_gt_cases 0 xu) as [Hu|Hu].
        * destruct (ub z2) as [zh| |] eqn:Eh.
          ...
          -- set (vzs := Z.max zlo2 (Z.max 1 (M + 2))).
             ...
             { pose proof (fband_ne (lb (x s)) (Fin xu) vzs Hz1v Hxle) as Hbn.
...
(*                                                                     *)
(*  A generic derivation ([closure_laws], zitv): any propagator that is *)
...
(** ** Truncated-integer division via the FLOOR solver + disjunction
    ...
    of the same operation lives in old_tdiv.v ([tdiv_fast_zitv3]). *)
(* ================================================================== *)
...
(* tdiv_rel at a point => fdiv_rel on the (mirrored) positive slice. *)
Lemma tdiv_rel_q1 : forall x y z, tdiv_rel x y z -> 0 <= y -> 0 < z -> fdiv_rel x y z.
Proof. intros x y z [Hnz Hq] Hy Hz. split; [exact Hnz | rewrite Hq; apply quot_eq_div; lia]. Qed.

Lemma tdiv_rel_q2 : forall x y z, tdiv_rel x y z -> 0 <= y -> z < 0 -> fdiv_rel (- x) y (- z).
Proof.
...
Lemma tdiv_rel_q3 : forall x y z, tdiv_rel x y z -> y <= 0 -> 0 < z -> fdiv_rel (- x) (- y) z.
Proof.
  intros x y z [Hnz Hq] Hy Hz. split; [lia | ].
  rewrite <- (quot_eq_div (- y) z) by lia.
...
Lemma tdiv_rel_q4 : forall x y z, tdiv_rel x y z -> y <= 0 -> z < 0 -> fdiv_rel x (- y) (- z).
Proof.
  intros x y z [Hnz Hq] Hy Hz. split; [lia | ].
  rewrite <- (quot_eq_div (- y) (- z)) by lia.
...
(* fdiv_rel on the (mirrored) positive slice => tdiv_rel at the original point. *)
Lemma sol_q1 : forall a b c, fdiv_rel a b c -> 0 <= b -> 1 <= c -> tdiv_rel a b c.
Proof. intros a b c [Hnz Hd] Hb Hc. split; [lia | rewrite Hd; symmetry; apply quot_eq_div; lia]. Qed.

Lemma sol_q2 : forall a b c, fdiv_rel a b c -> 0 <= b -> 1 <= c -> tdiv_rel (- a) b (- c).
Proof.
...
Lemma sol_q3 : forall a b c, fdiv_rel a b c -> 0 <= b -> 1 <= c -> tdiv_rel (- a) (- b) c.
Proof.
  intros a b c [Hnz Hd] Hb Hc. split; [lia | ].
...
Lemma sol_q4 : forall a b c, fdiv_rel a b c -> 0 <= b -> 1 <= c -> tdiv_rel a (- b) (- c).
Proof.
  intros a b c [Hnz Hd] Hb Hc. split; [lia | ].
  rewrite Z.quot_opp_l by lia. rewrite Z.quot_opp_r by lia.
...
Lemma contains_pos_y : forall s vx vy vz,
  in_zitv3 s vx vy vz -> 0 <= vy -> in_zitv3 (pos_y s) vx vy vz.
Proof.
...
Lemma contains_pos_y_inv : forall s vx vy vz,
...
Lemma contains_neg_y : forall s vx vy vz,
  in_zitv3 s vx vy vz -> vy <= 0 -> in_zitv3 (neg_y s) vx vy vz.
Proof.
  intros s vx vy vz (Hmx & Hmy & Hmz) Hy.
  destruct Hmy as [Hyl Hyu].
...
Lemma contains_neg_y_inv : forall s vx vy vz,
...
Lemma nonempty_sub : forall i j,
  leq_zinf (lb j) (lb i) -> leq_zinf (ub i) (ub j) -> is_not_bot_zitv i = true -> is_not_bot_zitv j = true.
Proof.
  intros [li ui] [lj uj]; cbn [lb ub]. unfold is_not_bot_zitv; cbn [lb ub].
...
Proof.
  ...
  destruct (negb (is_not_bot_zitv _)); [ cbn [x lb ub]; split; apply leq_zinf_refl | ].
  destruct (negb (is_not_bot_zitv _)); [ cbn [x lb ub]; split; apply leq_zinf_refl | ].
  cbn [x lb ub]. split; [apply leq_zinf_max_zinf_l | apply leq_zinf_min_zinf_l].
Qed.
...
Lemma fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw : forall s,
  is_not_bot_zitv3 (fdiv_pos_z_zitv3 s) = true ->
  is_not_bot_zitv (x s) = true /\ is_not_bot_zitv (y s) = true.
Proof.
...
Proof.
  ...
  assert (Hne : is_not_bot_zitv3 s = true) by (eapply nonempty_is_not_bot_zitv3; exact Hin).
  unfold tdiv_zitv3. rewrite Hne. cbn [negb].
...
Lemma geq_cw_implies_geq_join : forall a b c d t,
...
Proof.
  ...
  assert (Es : is_not_bot_zitv3 s = true) by (eapply nonempty_is_not_bot_zitv3; exact Hfin).
  destruct (not_bot_zitv3_distributes_cw s Es) as (Esx & Esy & Esz).
  assert (HneOut : is_not_bot_zitv3 (tdiv_zitv3 s) = true)
    by (eapply nonempty_is_not_bot_zitv3; apply tdiv_zitv3_soundness; [exact Hfin | exact Hfts]).
  unfold tdiv_zitv3 in *. rewrite Es in *. cbn [negb] in *.
  apply geq_cw_implies_geq_join; [ | | | | exact HneOut ].
  - (* Q1 *)
    intro HneQ. destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ HneQ) as [Hsx Hsy].
    apply fdiv_pos_z_zitv3_completeness.
  ...
  - (* Q2 *)
    ...
    destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ HneQ) as [Hsx Hsy].
    apply fdiv_pos_z_zitv3_completeness.
...
Proof.
  intros s Hne. unfold tdiv_zitv3 in Hne.
  destruct (is_not_bot_zitv3 s) eqn:Es; [ | cbn in Hne; congruence ].
  cbn [negb] in Hne.
  destruct (not_bot_zitv3_distributes_cw s Es) as (Esx & Esy & Esz).
  destruct (join_no_bot_implies_one_no_bot _ _ Hne) as [H1 | Hne2].
  - (* Q1 *)
    destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ H1) as [Hsx Hsy].
  ...
  - destruct (join_no_bot_implies_one_no_bot _ _ Hne2) as [H2 | Hne3].
    + (* Q2 *)
      rewrite ne_mir_xz in H2.
      destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ H2) as [Hsx Hsy].
    ...
    + destruct (join_no_bot_implies_one_no_bot _ _ Hne3) as [H3 | H4].
      * (* Q3 *)
        rewrite not_bot_neg_xy in H3.
        destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ H3) as [Hsx Hsy].
      ...
      * (* Q4 *)
        ...
        destruct (fdiv_zitv3_pos_z_not_bot_implies_not_bot_cw _ H4) as [Hsx Hsy].
...
Proof.
  intros s t Hct. destruct (is_not_bot_zitv3 (tdiv_zitv3 s)) eqn:E;
    [ exact (tdiv_zitv3_best_feasible s (tdiv_zitv3_not_bot_feasible s E) t Hct)
    | apply bot_is_leq_all_zitv3; exact E ].
Qed.

(* [tdiv_zitv3] is a CLOSURE OPERATOR (reductive, monotone, idempotent up to ~):
...
Proof. exact (proj1 (closure_laws tdiv_rel tdiv_zitv3 tdiv_zitv3_soundness (fun s _ => tdiv_zitv3_completeness s) tdiv_zitv3_not_bot_feasible)). Qed.
Theorem tdiv_zitv3_monotone : forall s t, leq_zitv3 s t -> leq_zitv3 (tdiv_zitv3 s) (tdiv_zitv3 t).
Proof. exact (proj1 (proj2 (closure_laws tdiv_rel tdiv_zitv3 tdiv_zitv3_soundness (fun s _ => tdiv_zitv3_completeness s) tdiv_zitv3_not_bot_feasible))). Qed.
Theorem tdiv_zitv3_idempotent : forall s,
  leq_zitv3 (tdiv_zitv3 (tdiv_zitv3 s)) (tdiv_zitv3 s) /\ leq_zitv3 (tdiv_zitv3 s) (tdiv_zitv3 (tdiv_zitv3 s)).
Proof. exact (proj2 (proj2 (closure_laws tdiv_rel tdiv_zitv3 tdiv_zitv3_soundness (fun s _ => tdiv_zitv3_completeness s) tdiv_zitv3_not_bot_feasible))). Qed.

(* Complete on singleton: on a fully-fixed store, a non-empty output forces the
   assignment to be an actual solution.  Immediate from [tdiv_zitv3_not_bot_feasible]
   (tdiv_zitv3 is the best propagator): a non-empty output makes the slice feasible,
...
Proof.
  ...
  destruct (tdiv_zitv3_not_bot_feasible s Hne) as (vx' & vy' & vz' & Hin & Hts).
  destruct Hin as (Hmx & Hmy & Hmz).

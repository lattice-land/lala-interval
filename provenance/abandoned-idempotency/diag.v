...
Definition rows_ok (ix1 iy iz0 : itv) : bool :=
  ...
  let base := andb (0 <? lo iz0) (andb (ileb ix1 (Cx iy iz0))
  ...
  ; chk ((a =? 0) && (b =? 0) && (d <? 0))%bool (zu <=? d - 1)
  ; chk ((a =? 0) && (b =? 0) && (0 <=? d))%bool (c + 1 <=? zl)
  ...
  ; chk ((a =? 0) && (0 <? b) && (c <? 0) && (d <=? 0))%bool (zu <=? cdiv (Z.min (-1) d) (b+1) - 1)
  ; chk ((a =? 0) && (0 <? b) && negb ((c <? 0) && (d <=? 0)) && (0 <? d) && (0 <=? c))%bool (Z.max 1 c / (b+1) + 1 <=? zl)
  ].

Theorem rows_grid :
  forallb (fun a => forallb (fun b => forallb (fun c => rows_ok a b c) itvs) itvs) itvs = true.

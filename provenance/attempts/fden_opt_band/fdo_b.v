...
(* Multiplicative bound lemmas used to discharge the band constraints of
   [fden] at the refined interval's own bounds. *)
...
Proof.
  intros ix iy iz Hsign Hab Hcd _ Hne.
  revert Hne. unfold fden; cbv zeta; unfold mem, sol.
  destruct (0 <? lo iz) eqn:Hzlb.
  - (* positive side: every w in the refined interval satisfies
       lo ix * w <= hi iy  and  lo iy + 1 <= (hi ix + 1) * w *)
    apply Z.ltb_lt in Hzlb.
    match goal with |- context[if ?g then _ else _] => destruct g eqn:Hk end.
    + cbn [lo hi]. intro Hne. exfalso; lia.
    + cbn [lo hi].
      ...
      { destruct Hk1 as [K|K]; [left; apply Z.eqb_neq in K; exact K
                               | right; apply Z.ltb_ge in K; exact K]. }
      assert (HkB : hi ix + 1 <> 0 \/ lo iy < 0).
      { destruct Hk2 as [K|K]; [left; apply Z.eqb_neq in K; exact K
                               | right; apply Z.leb_gt in K; exact K]. }
      clear Hk1 Hk2.
      assert (HA : forall w,
        (if lo ix <? 0 then cdiv (hi iy) (lo ix) else lo iz) <= w ->
        w <= (if 0 <? lo ix then hi iy / lo ix else hi iz) ->
        lo ix * w <= hi iy).
      { destruct (Z.ltb_spec (lo ix) 0) as [Ha|Ha].
        - intros w H1 _. pose proof (cdiv_mul_le_neg (hi iy) (lo ix) Ha). nia.
        - destruct (Z.ltb_spec 0 (lo ix)) as [Ha'|Ha'].
          + intros w _ H2. pose proof (Z.mul_div_le (hi iy) (lo ix) Ha'). nia.
          + intros w _ _.
            assert (E : lo ix = 0) by lia; rewrite E; destruct HkA as [K|K]; lia. }
      assert (HB : forall w,
        (if 0 <? hi ix + 1 then cdiv (lo iy + 1) (hi ix + 1) else lo iz) <= w ->
        w <= (if hi ix + 1 <? 0 then (lo iy + 1) / (hi ix + 1) else hi iz) ->
        lo iy + 1 <= (hi ix + 1) * w).
      { destruct (Z.ltb_spec 0 (hi ix + 1)) as [Hb|Hb].
        - intros w H1 _. pose proof (cdiv_mul_ge (lo iy + 1) (hi ix + 1) Hb). nia.
        - destruct (Z.ltb_spec (hi ix + 1) 0) as [Hb'|Hb'].
          + intros w _ H2. pose proof (div_mul_ge_neg (lo iy + 1) (hi ix + 1) Hb'). nia.
          + intros w _ _.
            assert (E : hi ix + 1 = 0) by lia; rewrite E; destruct HkB as [K|K]; lia. }
      match goal with |- ?L' <= ?U' -> _ =>
        remember L' as L eqn:HL; remember U' as U eqn:HU end.
      intro Hne.
      assert (H0L : 0 < L) by lia.
      assert (H0U : 0 < U) by lia.
      assert (HAL : lo ix * L <= hi iy) by (apply HA; lia).
      assert (HBL : lo iy + 1 <= (hi ix + 1) * L) by (apply HB; lia).
      assert (HAU : lo ix * U <= hi iy) by (apply HA; lia).
      assert (HBU : lo iy + 1 <= (hi ix + 1) * U) by (apply HB; lia).
      split.
      * destruct (witness_pos (lo ix) (hi ix) (lo iy) (hi iy) L H0L Hab Hcd HAL ltac:(lia))
          as (x & y & Hx & Hy & Hz0 & Hxy).
        exists x, y. tauto.
      * destruct (witness_pos (lo ix) (hi ix) (lo iy) (hi iy) U H0U Hab Hcd HAU ltac:(lia))
          as (x & y & Hx & Hy & Hz0 & Hxy).
        exists x, y. tauto.
  - (* negative side: every w in the refined interval satisfies
       lo iy <= lo ix * w  and  (hi ix + 1) * w <= hi iy - 1 *)
    apply Z.ltb_ge in Hzlb.
    destruct (hi iz <? 0) eqn:Hzub.
    + apply Z.ltb_lt in Hzub.
      match goal with |- context[if ?g then _ else _] => destruct g eqn:Hk end.
      * cbn [lo hi]. intro Hne. exfalso; lia.
      * cbn [lo hi].
        apply Bool.orb_false_iff in Hk; destruct Hk as [Hk1 Hk2].
        ...
        clear Hk1 Hk2.
        assert (HA : forall w,
          (if 0 <? lo ix then cdiv (lo iy) (lo ix) else lo iz) <= w ->
          w <= (if lo ix <? 0 then lo iy / lo ix else hi iz) ->
          lo iy <= lo ix * w).
        { destruct (Z.ltb_spec 0 (lo ix)) as [Ha|Ha].
          - intros w H1 _. pose proof (cdiv_mul_ge (lo iy) (lo ix) Ha). nia.
          - destruct (Z.ltb_spec (lo ix) 0) as [Ha'|Ha'].
            + intros w _ H2. pose proof (div_mul_ge_neg (lo iy) (lo ix) Ha'). nia.
            + intros w _ _.
              assert (E : lo ix = 0) by lia; rewrite E; destruct HkA as [K|K]; lia. }
        assert (HB : forall w,
          (if hi ix + 1 <? 0 then cdiv (hi iy - 1) (hi ix + 1) else lo iz) <= w ->
          w <= (if 0 <? hi ix + 1 then (hi iy - 1) / (hi ix + 1) else hi iz) ->
          (hi ix + 1) * w <= hi iy - 1).
        { destruct (Z.ltb_spec (hi ix + 1) 0) as [Hb|Hb].
          - intros w H1 _. pose proof (cdiv_mul_le_neg (hi iy - 1) (hi ix + 1) Hb). nia.
          - destruct (Z.ltb_spec 0 (hi ix + 1)) as [Hb'|Hb'].
            + intros w _ H2. pose proof (Z.mul_div_le (hi iy - 1) (hi ix + 1) Hb'). nia.
            + intros w _ _.
              assert (E : hi ix + 1 = 0) by lia; rewrite E; destruct HkB as [K|K]; lia. }
        match goal with |- ?L' <= ?U' -> _ =>
          remember L' as L eqn:HL; remember U' as U eqn:HU end.
        intro Hne.
        assert (H0U : U < 0) by lia.
        assert (H0L : L < 0) by lia.
        assert (HAL : lo iy <= lo ix * L) by (apply HA; lia).
        assert (HBL : (hi ix + 1) * L <= hi iy - 1) by (apply HB; lia).
        assert (HAU : lo iy <= lo ix * U) by (apply HA; lia).
        assert (HBU : (hi ix + 1) * U <= hi iy - 1) by (apply HB; lia).
        split.
        { destruct (realize_neg (lo ix) (hi ix) (lo iy) (hi iy) L H0L Hab Hcd HAL ltac:(lia))
            as (x & y & Hx & Hy & Hz0 & Hxy).
          exists x, y. tauto. }
        { destruct (realize_neg (lo ix) (hi ix) (lo iy) (hi iy) U H0U Hab Hcd HAU ltac:(lia))
            as (x & y & Hx & Hy & Hz0 & Hxy).
          exists x, y. tauto. }
    + apply Z.ltb_ge in Hzub. intro Hne. exfalso. destruct Hsign as [H|H]; lia.
Qed.

...
| Floor division | $x = \mathit{fdiv}(y,z)$ | `x.fdiv(y, z)`[^1] | `y.fdiv_num(x, z)`[^1] and `z.fdiv_den(x,y)` |
| Ceiling division | $x = \mathit{cdiv}(y,z)$ | `x.cdiv(y, z)`[^1] | `y.cdiv_num(x, z)`[^1] and `z.cdiv_den(x,y)` |
| Truncated division | $x = \mathit{tdiv}(y,z)$ | `x.tdiv(y, z)`[^1] | `y.tdiv_num(x, z)`[^1] and `z.tdiv_den(x,y)` |
| Euclidean division | $x = \mathit{ediv}(y,z)$ | `x.ediv(y, z)`[^1] | `y.ediv_num(x, z)`[^1] and `z.ediv_den(x,y)` |
| Maximum | $x = \mathit{max}(y, z)$ | `x.max(y, z)` | `y.maxb(x,z)` and `z.maxb(x,y)` |
| Minimum | $x = \mathit{min}(y, z)$ | `x.min(y, z)` | `y.minb(x,z)` and `z.minb(x,y)` |
